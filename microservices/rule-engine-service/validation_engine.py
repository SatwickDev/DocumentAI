...
class ValidationEngine:
    ...
    @staticmethod
    def validate_min_length(field, params, data):
        if field not in data:
            return False, f"Field {field} not found"
        value_str = str(data[field]).strip()
        if not value_str:
            return False, f"Field {field} is empty"
        if len(value_str) < params['min_length']:
            return False, f"Too short: {len(value_str)} characters, must be at least {params['min_length']}"
        return True, f"Valid length: {len(value_str)} characters"
    ...
    def parse_english_rule(self, rule_text: str, field_tag: str) -> Dict[str, Any]:
        ...
        # 8. Minimum length (at least X characters)
        m = re.search(r'at least (\d+) characters?', text_lower)
        if m:
            rule_info['type'] = 'min_length'
            rule_info['parameters']['min_length'] = int(m.group(1))
            return rule_info

        # 9. Greater/Less/Between (but not character-related)
        if 'greater than' in text_lower or 'more than' in text_lower or 'above' in text_lower or 'exceeds' in text_lower or ('at least' in text_lower and 'character' not in text_lower) or 'not less than' in text_lower:
        ...
        for rule in config_rules['rules']:
            ...
            try:
                ...
                result = {
                    ...
                    'passed': is_valid,  # Frontend expects 'passed'
                    'rule_description': rule['original_text'],  # Frontend expects 'rule_description'
                    'applied_value': json_data.get(field, 'NOT_FOUND'),  # Frontend expects 'applied_value'
                    'message': message,
                ...
                result = {
                    ...
                    'rule_type': rule_info['type'],
                    'original_rule': rule['original_text'],
                    'passed': False,  # Frontend expects 'passed'
                    'rule_description': rule['original_text'],  # Frontend expects 'rule_description'
                    'applied_value': json_data.get(field, 'NOT_FOUND'),  # Frontend expects 'applied_value'
